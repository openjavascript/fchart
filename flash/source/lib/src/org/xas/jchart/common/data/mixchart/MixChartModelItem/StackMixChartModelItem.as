package org.xas.jchart.common.data.mixchart.MixChartModelItem
{
	...
	public class StackMixChartModelItem extends BaseMixChartModelItem
	{
		
		public function StackMixChartModelItem( _index:int, _params:Object, _config:Config )
		{
			super( _index, _params, _config );
		}
		...
		override protected function calcMaxNum( _series:Array = null ):Number{
			var _r:Number = 0, _tmp:Number;
			_series = _series || displaySeries;
			
			if( _series && _series.length && _series[0].data && _series[0].data.length  ){
				
				Common.each( _series[0].data, function( _k:int, _item:Number ):void {
					_tmp = 0;
					Common.each( _series, function( _sk:int, _sitem:Object ):void {
