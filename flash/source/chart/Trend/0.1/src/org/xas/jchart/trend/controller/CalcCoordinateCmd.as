package org.xas.jchart.trend.controller
{
	...
	public class CalcCoordinateCmd extends SimpleCommand implements ICommand
	{
		...
		private function calcGraphic():void{
			...
			_config.c.paths = [];
			var _dataFlow:Array = _config.dataFlow;
			if( !( _dataFlow && _dataFlow.length ) ) return;

			_config.c.partWidth = _config.c.itemWidth / _dataFlow.length;
			var _cmd:Vector.<int> = new Vector.<int>
				, _path:Vector.<Number> = new Vector.<Number>
				, _positions:Array = [];
			
			Common.each( _dataFlow, function( _sk:int, _item:Object ):void{
				var _sp:Point = _config.c.hpointReal[ _sk ].start as Point
					, _ep:Point = _config.c.hpointReal[ _sk ].end as Point
					, _h:Number, _y:Number
					, _dataHeight:Number
					, _num:Number = _item.c;
				...
				_cmd.push( _sk === 0 ? 1 : 2 );
				_path.push( _sp.x, _y );
				_positions.push( { x: _sp.x, y: _y, value: _item.c } );
			});
